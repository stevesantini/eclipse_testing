...
setInterval(() => {getAndPrintTickers(publicExchanges, 'BTC-USD').then(() => {return getAndPrintTickers(publicExchanges, 'ETH-USD');
    }).catch((err) => {logger.log('error', err.message, err);});}, 5000);
setInterval(getAndPrintTickers(publicExchanges, 'BCH-USD').catch((err) => {logger.log('error', err.message, err)});,5000);
